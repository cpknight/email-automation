...
try:
    ...
    SOURCE_FOLDER = config['archive_correspondence']['source_folder']
    DEST_FOLDER = config['archive_correspondence']['dest_folder']
    BATCH_SIZE = int(config['archive_correspondence']['batch_size'])
...
def process_correspondence():
    ...
    while batch_start <= total_msgs and not stop_processing:
        ...
        if not messages_to_archive:
            console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [info]No messages to archive in this batch[/info]")
...
if __name__ == "__main__":
    console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [info]Starting correspondence archiving script...[/info]")
