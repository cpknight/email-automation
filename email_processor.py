...
try:
    ...
    SOURCE_FOLDER = config['processor']['source_folder']
    DEST_FOLDER = config['processor']['dest_folder']
    BATCH_SIZE = int(config['processor']['batch_size'])
...
def move_message(mail, msg_id, uid, source_folder, dest_folder, dest_signatures, signature):
    max_retries = 3
    for attempt in range(max_retries):
        try:
            ...
            if status == "OK":
                time.sleep(5)  # Match archiving scripts' delay
                mail.store(msg_id, "+FLAGS", "\\Deleted")
                console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [info]Moved msg {msg_id} (UID {uid}) to {dest_folder}[/info]")
                return True
            else:
                console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [warning]Failed to copy msg {msg_id} (UID {uid}) to {dest_folder} on attempt {attempt + 1}[/warning]")
                if attempt < max_retries - 1:
                    time.sleep(5)  # Wait before retry
                    continue
                return False
        except imaplib.IMAP4.error as e:
            console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [warning]IMAP error moving msg {msg_id} (UID {uid}) to {dest_folder} on attempt {attempt + 1}: {e}[/warning]")
            if attempt < max_retries - 1:
                time.sleep(5)  # Wait before retry
                continue
            return False
...
def process_emails():
    ...
    console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [info]Cutoff date: {cutoff_date.strftime('%Y-%m-%d %H:%M:%S UTC')}[/info]")
    batch_start = 1
    moved_count = 0
    
    dest_signatures = {sig: (uid, msg_id, date) for sig, (uid, msg_id, date) in get_message_signatures_and_dates(mail, DEST_FOLDER).items()}
    dest_signature_keys = set(dest_signatures.keys())
    ...
    while batch_start <= total_msgs and not stop_processing:
        ...
        messages_to_move = [
            (signature, uid, msg_id, msg_date)
            for signature, (uid, msg_id, msg_date) in batch_data.items()
            if msg_date < cutoff_date
        ]
        
        if not messages_to_move:
            console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [info]No messages older than 24 hours in this batch[/info]")
            batch_start += BATCH_SIZE
            continue
        
        console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [info]Moving {len(messages_to_move)} messages to {DEST_FOLDER}...[/info]")
        with Progress(
            TextColumn("[progress.description]{task.description}"),
            BarColumn(),
            console=console,
            ...
            task = progress.add_task(f"[highlight]Moving to {DEST_FOLDER}[/highlight]", total=len(messages_to_move))
            for signature, uid, msg_id, msg_date in messages_to_move:
                if stop_processing:
                    break
                console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [info]Processing msg {msg_id} (UID {uid}) with date {msg_date.strftime('%Y-%m-%d %H:%M:%S UTC')}[/info]")
                success = move_message(mail, msg_id, uid, SOURCE_FOLDER, DEST_FOLDER, dest_signature_keys, signature)
                if success:
                    moved_count += 1
                    dest_signature_keys.add(signature)
                    progress.update(task, advance=1)
        
        mail.select(SOURCE_FOLDER)
        mail.expunge()
        console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [info]Batch complete: {moved_count} messages moved[/info]")
        batch_start += BATCH_SIZE

    if stop_processing:
        console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [warning]Processing aborted by user[/warning]")
    else:
        console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [success]Processing complete: {moved_count} messages moved to {DEST_FOLDER}[/success]")
    mail.logout()

if __name__ == "__main__":
    console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [info]Starting email processor script...[/info]")
    process_emails()
