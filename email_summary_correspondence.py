...
try:
    ...
    CORRESPONDENCE_FOLDER = config['summary_correspondence']['correspondence_folder']
    DRAFTS_FOLDER = config['summary_correspondence']['drafts_folder']
    BATCH_SIZE = int(config['summary_correspondence']['batch_size'])
...
def classify_correspondence(msg):
    from_addr = msg.get("From", "").lower()
    subject = msg.get("Subject", "").lower()
    
    personal_keywords = ["re:", "fwd:", "hi", "hello", "dear"]
    business_domains = [".co", ".org", ".edu", ".gov", "@company", "@work"]
    
    if any(keyword in subject for keyword in personal_keywords) or \
       re.match(r'[^@]+@[^@]+\.[a-z]{2,}$', from_addr) and not any(d in from_addr for d in business_domains):
        return "Personal"
    if any(domain in from_addr for domain in business_domains) or "invoice" in subject or "meeting" in subject:
        return "Business"
    return "Other"
        ...
        with Progress(
            ...
            while i < len(fetch_data) and msg_index < end:
                ...
                try:
                    ...
                    classification = classify_correspondence(msg)
...
def generate_pie_chart(classifications):
    ...
    plt.title("Correspondence Classifications")
...
def create_draft_summary(mail, correspondence_data):
    timestamp = get_utc_timestamp()
    subject = f"Correspondence Folder Summary as at {timestamp}"
    
    if not correspondence_data:
        return
    
    dates = [data['date'] for data in correspondence_data.values()]
    classifications = [data['classification'] for data in correspondence_data.values()]
    senders = [data['from'] for data in correspondence_data.values()]
    discrepancies = sum(1 for data in correspondence_data.values() if data['from'] != data['return_path'])
    injections = sum(1 for data in correspondence_data.values() if data['subject_injection'] or data['from_injection'] or data['return_path_injection'])
    ...
    total_msgs = len(correspondence_data)
    
    pie_chart_img = generate_pie_chart(classifications)
    
    table_rows = []
    for idx, data in enumerate(correspondence_data.values()):
    ...
    <body>
        <div class="container">
            <h1>Correspondence Folder Summary</h1>
            <p>This report provides an overview of correspondence activity in your inbox as of {timestamp}. Spanning the period from {date_range_start} to {date_range_end}, our system analyzed a total of <b style='color: #d81b60;'>{total_msgs}</b> messages. These have been categorized into distinct types, with <b style='color: #d81b60;'>{class_counts.get('Personal', 0)}</b> identified as personal communications, <b style='color: #d81b60;'>{class_counts.get('Business', 0)}</b> classified as business-related, and <b style='color: #d81b60;'>{class_counts.get('Other', 0)}</b> falling into an miscellaneous category. A total of <b style='color: #d81b60;'>{discrepancies}</b> messages showed discrepancies between their 'From' and 'Return-Path' headers, potentially indicating misidentification risks. Additionally, <b style='color: #d81b60;'>{injections}</b> instances of potential code injections were detected in message fields, suggesting possible security concerns. The most frequent senders include {sender_text}, highlighting key correspondents in this period.</p>
            
            <h2>Correspondence Classification Breakdown</h2>
            <img src="data:image/png;base64,{pie_chart_img}" alt="Correspondence Classifications Pie Chart">
            
            <h2>Correspondence Messages</h2>
            {table_html}
        </div>
    </body>
    </html>
    ...
    msg['To'] = USERNAME
    msg['Subject'] = subject
    msg['List-ID'] = "CorrespondenceSummary"
...
def process_correspondence_folder():
    ...
    mail.select(CORRESPONDENCE_FOLDER)
    status, messages = mail.search(None, 'ALL')
    if status != "OK" or not messages[0]:
        console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [warning]Failed to search {CORRESPONDENCE_FOLDER}[/warning]")
        mail.logout()
        return
    total_msgs = len(messages[0].split())
    console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [info]Found {total_msgs} messages in {CORRESPONDENCE_FOLDER}[/info]")

    if total_msgs == 0:
        console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [info]No messages in {CORRESPONDENCE_FOLDER}, skipping summary[/info]")
        mail.logout()
        return
    ...
    while batch_start <= total_msgs and not stop_processing:
        ...
        batch_data = get_message_signatures_and_headers(mail, CORRESPONDENCE_FOLDER, start=batch_start, end=batch_end)
        all_correspondence_data.update(batch_data)
    ...
    create_draft_summary(mail, all_correspondence_data)
    console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [success]Processing complete[/success]")
    mail.logout()

if __name__ == "__main__":
    console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [info]Starting correspondence summary script...[/info]")
    process_correspondence_folder()
