...
def signal_handler(sig, frame):
    ...
    if not stop_processing:
        console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [warning]Abortiels requested (Ctrl+C detected), finishing current message...[/warning]")
...
try:
    ...
    NOTIFICATIONS_FOLDER = config['summary_notifications']['notifications_folder']
    DRAFTS_FOLDER = config['summary_notifications']['drafts_folder']
    BATCH_SIZE = int(config['summary_notifications']['batch_size'])
...
def classify_notification(msg):
    ...
    service_keywords = ["update", "maintenance", "account", "login", "verify"]
    marketing_keywords = ["offer", "discount", "sale", "promo", "free"]
    alert_keywords = ["alert", "warning", "urgent", "security"]
    
    if any(keyword in subject or keyword in from_addr for keyword in service_keywords):
        return "Service"
    if any(keyword in subject or keyword in from_addr for keyword in marketing_keywords):
        return "Marketing"
    if any(keyword in subject or keyword in from_addr for keyword in alert_keywords):
        return "Alert"
    return "Service"  # Default to Service for notifications
        ...
        with Progress(
            ...
            while i < len(fetch_data) and msg_index < end:
                ...
                try:
                    ...
                    classification = classify_notification(msg)
...
def generate_pie_chart(classifications):
    ...
    plt.title("Notifications Classifications")
...
def create_draft_summary(mail, notifications_data):
    timestamp = get_utc_timestamp()
    subject = f"Notifications Folder Summary as at {timestamp}"
    
    if not notifications_data:
        return
    
    dates = [data['date'] for data in notifications_data.values()]
    classifications = [data['classification'] for data in notifications_data.values()]
    senders = [data['from'] for data in notifications_data.values()]
    discrepancies = sum(1 for data in notifications_data.values() if data['from'] != data['return_path'])
    injections = sum(1 for data in notifications_data.values() if data['subject_injection'] or data['from_injection'] or data['return_path_injection'])
    ...
    total_msgs = len(notifications_data)
    
    pie_chart_img = generate_pie_chart(classifications)
    
    table_rows = []
    for idx, data in enumerate(notifications_data.values()):
    ...
    <body>
        <div class="container">
            <h1>Notifications Folder Summary</h1>
            <p>This report details notification activity in your inbox as of {timestamp}. Covering the period from {date_range_start} to {date_range_end}, our system analyzed a total of <b style='color: #d81b60;'>{total_msgs}</b> messages. These have been categorized into distinct types, with <b style='color: #d81b60;'>{class_counts.get('Service', 0)}</b> identified as service-related notices, <b style='color: #d81b60;'>{class_counts.get('Marketing', 0)}</b> classified as marketing promotions, and <b style='color: #d81b60;'>{class_counts.get('Alert', 0)}</b> flagged as alerts. A total of <b style='color: #d81b60;'>{discrepancies}</b> messages showed discrepancies between their 'From' and 'Return-Path' headers, which may indicate delivery issues. Additionally, <b style='color: #d81b60;'>{injections}</b> instances of potential code injections were detected in message fields, suggesting possible security risks. The most frequent senders include {sender_text}, identifying key sources of notifications in this period.</p>
            
            <h2>Notifications Classification Breakdown</h2>
            <img src="data:image/png;base64,{pie_chart_img}" alt="Notifications Classifications Pie Chart">
    ...
    msg['To'] = USERNAME
    msg['Subject'] = subject
    msg['List-ID'] = "NotificationsSummary"
...
def process_notifications_folder():
    global stop_processing
    mail = connect_to_imap()
    
    mail.select(NOTIFICATIONS_FOLDER)
    status, messages = mail.search(None, 'ALL')
    if status != "OK" or not messages[0]:
        console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [warning]Failed to search {NOTIFICATIONS_FOLDER}[/warning]")
        mail.logout()
        return
    total_msgs = len(messages[0].split())
    console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [info]Found {total_msgs} messages in {NOTIFICATIONS_FOLDER}[/info]")

    if total_msgs == 0:
        console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [info]No messages in {NOTIFICATIONS_FOLDER}, skipping summary[/info]")
        mail.logout()
        return
    ...
    while batch_start <= total_msgs and not stop_processing:
        ...
        batch_data = get_message_signatures_and_headers(mail, NOTIFICATIONS_FOLDER, start=batch_start, end=batch_end)
        all_notifications_data.update(batch_data)
    ...
    create_draft_summary(mail, all_notifications_data)
    console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [success]Processing complete[/success]")
    mail.logout()

if __name__ == "__main__":
    console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [info]Starting notifications summary script...[/info]")
    process_notifications_folder()
