...
try:
    ...
    SOURCE_FOLDER = config['archive_sent']['source_folder']
    DEST_FOLDER = config['archive_sent']['dest_folder']
    BATCH_SIZE = int(config['archive_sent']['batch_size'])
...
def move_message_to_archive(mail, msg_id, uid, source_folder, dest_folder):
    max_retries = 3
    for attempt in range(max_retries):
        try:
            ...
            if status == "OK":
                time.sleep(5)  # Sync delay for Proton Bridge
...
def process_sent():
    ...
    while batch_start <= total_msgs and not stop_processing:
        ...
        if not messages_to_archive:
            console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [info]No messages older than 48 hours in this batch[/info]")
            batch_start += BATCH_SIZE
            continue
        
        console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [info]Archiving {len(messages_to_archive)} messages...[/info]")
        with Progress(
            TextColumn("[progress.description]{task.description}"),
            BarColumn(),
            console=console,
            ...
            task = progress.add_task(f"[highlight]Moving to {DEST_FOLDER}[/highlight]", total=len(messages_to_archive))
            for signature, uid, msg_id in messages_to_archive:
                if stop_processing:
                    break
                success = move_message_to_archive(mail, msg_id, uid, SOURCE_FOLDER, DEST_FOLDER)
                if success:
                    moved_count += 1
                    progress.update(task, advance=1)
        ...
        mail.select(SOURCE_FOLDER)
        mail.expunge()
        console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [info]Batch complete: {moved_count} moved to {DEST_FOLDER}[/info]")
        batch_start += BATCH_SIZE

    if stop_processing:
        console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [warning]Processing aborted by user[/warning]")
    else:
        console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [success]Processing complete: {moved_count} messages archived[/success]")
    mail.logout()

if __name__ == "__main__":
    console.print(f"[grey50][{get_utc_timestamp()}][/grey50] [info]Starting sent archiving script...[/info]")
    process_sent()
